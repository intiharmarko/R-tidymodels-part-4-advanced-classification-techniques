#' Assignment - Bank Marketing - Functions



#' This functions cleans initial table:
#' - converts target column to factor
#' - converts month column to numeric months
#' - adds flag if client contacted in previous marketing campaign
#' - converts character columns to factors
#' - remove variable "duration"
#' 
#' @param df_ A data frame of imported initial table.
#'
#' @return df_ A data frame - After cleaning steps applied.
#'
clean_df <- function(df_ = df){
  
  df_ <- df_ %>% 
    mutate(y = if_else(y == "yes", 1, 0),
           y = factor(y, levels = c(1, 0))) %>% 
    mutate(month = match(tolower(month), tolower(month.abb))) %>% 
    mutate(contacted_prev_camp = if_else(pdays == -1, "yes", "no")) %>% 
